"""Fixed availability calculation services for barber shop scheduling system."""
...
class AvailabilityService:
    ...
    def get_available_slots(
        ...
        """
        # Cache key for this request - include current date to avoid stale cache
        cache_key = f"availability_{professional_id}_{service_id}_{page}_{timezone.now().date()}"
    ...
    def _get_available_slots_for_day(
        ...
        # Adjust for current time if it's today - FIXED LOGIC
        if target_date == timezone.now().date():
            now = timezone.now()
            if now.time() > work_start.time():
        ...
        free_intervals = AvailabilityService._subtract_intervals_fixed(
    ...
    @staticmethod
    def _get_busy_intervals(profissional: Profissional, target_date: date) -> List[TimeSlot]:
    ...
    @staticmethod
    def _merge_overlapping_intervals(intervals: List[TimeSlot]) -> List[TimeSlot]:
        """Merge overlapping intervals to simplify subtraction logic."""
        if not intervals:
            return []
    ...
    @staticmethod
    def _subtract_intervals_fixed(
        base_interval: TimeSlot, 
        intervals_to_subtract: List[TimeSlot]
    ) -> List[TimeSlot]:
        """
        Fixed version: Subtract multiple intervals from a base interval.
        Handles overlapping intervals correctly by merging them first.
        """
        if not intervals_to_subtract:
            return [base_interval]
        
        # Merge overlapping intervals to handle them correctly
        merged_intervals = AvailabilityService._merge_overlapping_intervals(intervals_to_subtract)
        
        result = []
        current_start = base_interval.start
        
        for interval in merged_intervals:
            # If interval doesn't overlap with base, skip it
